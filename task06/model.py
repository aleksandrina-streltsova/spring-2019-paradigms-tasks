...
class Conditional(ASTNode):
    ...
    def evaluate(self, scope):
        ...
        if condition_result:
            return execute_expressions(self.if_true, scope)
        return execute_expressions(self.if_false, scope)
...
class FunctionCall(ASTNode):
    ...
    def evaluate(self, scope):
        ...
        return execute_expressions(function.body, call_scope)
...
class BinaryOperation(ASTNode):
    ...
    def evaluate(self, scope):
        lnum = self.lhs.evaluate(scope)
        rnum = self.rhs.evaluate(scope)
        if self.op == '+':
            return Number(lnum.value + rnum.value)
        if self.op == '-':
            return Number(lnum.value - rnum.value)
        if self.op == '*':
            return Number(lnum.value * rnum.value)
        ...
        if self.op == '%':
            return Number(lnum.value % rnum.value)
        if self.op == '==':
            return Number(int(lnum == rnum))
        if self.op == '!=':
            return Number(int(lnum != rnum))
        if self.op == '<':
            return Number(int(lnum.value < rnum.value))
        if self.op == '>':
            return Number(int(lnum.value > rnum.value))
        if self.op == '<=':
            return Number(int(lnum.value <= rnum.value))
        if self.op == '>=':
            return Number(int(lnum.value >= rnum.value))
        if self.op == '&&':
            return Number(int(bool(lnum and rnum)))
        if self.op == '||':
            return Number(int(bool(lnum or rnum)))
...
class UnaryOperation(ASTNode):
    ...
    def evaluate(self, scope):
        expr_result = self.expr.evaluate(scope)
        if self.op == '!':
            return Number(0 if expr_result else 1)
        if self.op == '-':
            return Number(-expr_result.value)


def execute_expressions(expressions, scope):
    result = Number(0)
